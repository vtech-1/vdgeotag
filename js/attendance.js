...
function _attKey() {
    // serverNow(): the local cache must be keyed to the real day, or
    // changing the phone's date would surface a different day's punches.
    return `${STATE.currentUser?.id}_${formatDateISO(serverNow())}`;
}
...
// THE SERVER DECIDES THE TIME. The device's clock is user-settable, so a
// supervisor could set their phone to 07:00 and punch in "on time". The
// time/date sent here are only `deviceTime`, used server-side to MEASURE
// the discrepancy — never to record it. What gets stored and shown is
...
async function _recordPunch(punchType, when, auto) {
    ...
    const deviceTimeLabel = formatTime(when);
    ...
    const punchData = {
        ...
        auto: !!auto,
        // For skew detection only. ISO 8601 carries the device's timezone
        // offset, so the server can compare absolute instants.
        deviceTime: when.toISOString(),
    };
    ...
    if (res.ok && res.data?.success) {
        ...
        const time   = res.data.serverTime || deviceTimeLabel;
        const status = res.data.status ? ` (${res.data.status})` : '';

        STATE.attendanceToday[sessionKey] = {
            time,
            lat: punchData.lat,
            lon: punchData.lon,
            auto: !!auto,
        ...
        showMessage(`✅ Attendance recorded at ${time}${status}${tag}`, 'success');

        // Tell them plainly if their phone clock is wrong. The punch was
        // recorded correctly regardless, but a wrong clock makes the app
        // look broken later — better they know now.
        const skew = res.data.clockSkewMin;
        if (typeof skew === 'number' && Math.abs(skew) > 5) {
            showMessage(
                `⚠️ Your phone clock is ${Math.abs(skew)} min ${skew > 0 ? 'ahead' : 'behind'}. ` +
                `Official time ${time} was used.`,
                'error'
            );
        }
    } else {
        // OFFLINE. Store the device's time as a provisional record so the
        // UI stays usable, and mark it pending — it is NOT authoritative
        // and the server will assign the real time when it syncs.
        STATE.attendanceToday[sessionKey] = {
            time: deviceTimeLabel,
            lat: punchData.lat,
            lon: punchData.lon,
            auto: !!auto,
            pending: true,
        ...
        showMessage(
            `⚠️ Saved on this phone at ${deviceTimeLabel}. Official time is set when it reaches the server.`,
            'error'
        );
    }
...
function recordPunchIn() {
    ...
    _recordPunch(info.inType, serverNow(), false);
}
...
function recordPunchOut() {
    ...
    if (a.in1 && !a.out1) { _recordPunch('1st_out', serverNow(), false); return; }
    if (a.in2 && !a.out2) { _recordPunch('2nd_out', serverNow(), false); return; }
}
