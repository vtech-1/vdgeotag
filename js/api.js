...
// ── Server time (public — corrects a wrong device clock) ──────
// Round-trip latency is halved off the result: the server's timestamp was
// generated roughly midway through the request, so the response is already
// that much "old" by the time it lands.
async function apiSyncServerTime() {
    const started = Date.now();
    const res = await apiGet('getServerTime', {});
    if (!res.ok || !res.data?.epochMs) return false;

    const halfTrip = (Date.now() - started) / 2;
    setServerTimeOffset(res.data.epochMs + halfTrip);
    return true;
}
