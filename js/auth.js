...
// A saved session carries the role as it was AT LOGIN TIME. When an admin
// changes someone's role — or a new role like 'serviceadmin' is introduced
// — an already-logged-in device keeps showing the old screen until the
// session expires, which looks exactly like the feature not working.
// Re-check the directory on startup and re-route if it has changed.
async function refreshSessionRole() {
    if (!STATE.currentUser) return false;

    const res = await apiGetEmployees();
    if (!res.ok || !Array.isArray(res.data?.rows)) return false;   // offline — keep the cached role

    const emp = res.data.rows.find(e => String(e.id) === String(STATE.currentUser.id));
    if (!emp) return false;

    if (emp.role === STATE.currentUser.role && emp.name === STATE.currentUser.name) {
        return false;
    }

    STATE.currentUser.role = emp.role;
    STATE.currentUser.name = emp.name;
    ...
    } catch (e) { /* session rewrite is best-effort */ }

    return true;   // caller re-routes
}
...
function goToRolePage() {
    ...
    if (STATE.currentUser.role === 'admin') {
        showPage('page-admin');
        initAdminDashboard();
    } else if (STATE.currentUser.role === 'serviceadmin') {
        showPage('page-serviceadmin');
    } else if (STATE.currentUser.role === 'service') {
