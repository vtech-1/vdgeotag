...
const CONFIG = {
    ...
    // Accept a fix up to 60 s old. This used to be 0, which on many Android
    // phones means "reject the cached/network fix and wait for a pure
    // satellite lock" — indoors or under shade that lock never arrives, so
    // the app sat on "Getting GPS signal..." and never captured a position.
    GPS_MAX_AGE: 60000,

    // If no high-accuracy fix has landed within this long, fall back to a
    // coarse (network/cell/wifi) fix so a photo is never stamped with
    // "GPS unavailable" just because the sky isn't visible.
    GPS_COARSE_FALLBACK_MS: 12000,

    // A fix worse than this is still used, but flagged as approximate.
