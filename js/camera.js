...
//
// These elements live inside #page-supervisor. The service team's
// "Report an Issue" page has its OWN camera (see inspections.js) because
// this one is hidden whenever that page is showing.
// ============================================================

let _cameraCaptureMode = 'report';

// True only for the modes that drive the attendance toggle.
function _isPunchMode(mode) {
    return mode === 'punch-in' || mode === 'punch-out';
}
...
async function openCamera(mode = 'report') {
    ...
    if (STATE.videoStream) {
        ...
        if (_isPunchMode(mode)) cancelPunchCapture();
        else _cameraCaptureMode = 'report';
    }
}
...
function addPhotoOverlay(ctx, w, h) {
    ...
    const gps  = STATE.currentPosition
        ? `GPS: ${STATE.currentPosition.latitude.toFixed(4)}, ${STATE.currentPosition.longitude.toFixed(4)}` +
          (STATE.currentPosition.accuracy > CONFIG.GPS_ACCURACY_WARN_M
              ? ` (±${Math.round(STATE.currentPosition.accuracy)}m)` : '')
        : 'GPS unavailable';
    const loc  = STATE.currentLocationName || 'Location detecting...';
    // serverNow(): the stamp burned into the photo is evidence, so it must
    // not be movable by changing the phone's clock. When the server time was
    // never reached, serverNow() IS the phone clock — mark the stamp rather
    // than pass an unverified time off as the official one.
    const ts   = formatTimestampIST(serverNow()) +
                 (isServerTimeKnown() ? ' IST' : ' (device time)');
...
function retakePhoto() {
    ...
    // supervisor can slide the toggle again to retry. An inspection retake
    // just stays in inspection mode for the next shot.
    if (_isPunchMode(_cameraCaptureMode)) cancelPunchCapture();
}
