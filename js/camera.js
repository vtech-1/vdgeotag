...
//   'inspection' — service team filing a site inspection (inspections.js)
//
// Only the two punch modes touch the attendance toggle; 'inspection' must
// never call the punch cleanup helpers or it would flip the toggle for an
// unrelated photo.
// ============================================================
...
async function openCamera(mode = 'report') {
    ...
    if (title) {
        title.textContent = mode === 'punch-in'   ? '📷 Punch In — Take a Photo'
                           ...
                           : mode === 'inspection' ? '📷 Site Inspection — Take a Photo'
                           : '📷 Photo Capture';
    }
...
function confirmPhoto() {
    ...
    if (_cameraCaptureMode === 'inspection') {
        // Hand the photo to the inspection form and close the camera. The
        // attendance toggle is deliberately untouched — this is not a punch.
        onInspectionPhotoReady(STATE.capturedPhoto);
        STATE.capturedPhoto = null;
