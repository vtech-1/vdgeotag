...
function switchAdminTab(tab) {
    ...
    // .admin-tab-* classes, and an unscoped selector would toggle its tabs
    // too.
    document.querySelectorAll('#page-admin .admin-tab-btn').forEach(b => {
        b.classList.toggle('active', b.dataset.tab === tab);
    });
    document.querySelectorAll('#page-admin .admin-tab-panel').forEach(p => {
        p.classList.toggle('active', p.id === 'tab-' + tab);
    });
    // Load on first switch to that tab
    if (tab === 'reports') loadAdminReports();
    if (tab === 'complaints') loadAdminComplaints();
    if (tab === 'supervisors') loadSupervisorsList();
    if (tab === 'inspections') loadInspections('adminInspectionsBody', {
        params: adminInspectionDateParams(),
    });
}

function adminInspectionDateParams() {
    const from = document.getElementById('insFilterFrom')?.value || '';
    const to   = document.getElementById('insFilterTo')?.value   || '';
    const params = {};
...
function loadAdminInspections() {
    loadInspections('adminInspectionsBody', { params: adminInspectionDateParams() });
}
...
async function loadAdminComplaints() {
    tableLoading('adminComplaintsBody', CMP_TABLE_COLS, '⏳ Loading complaints...');

    const res = await apiFetchComplaints(adminComplaintDateParams());
    if (!res.ok || !Array.isArray(res.data?.rows)) {
        tableLoading('adminComplaintsBody', CMP_TABLE_COLS, '⚠️ Failed to load complaints');
        return;
    }

    STATE.adminComplaints = res.data.rows;
    fillCmpSupervisorFilter('cmpFilterSupervisor', res.data.rows);
    renderAdminComplaints();
}

// Dates are sent ONLY when the admin picks them — otherwise the backend
// applies its fast default (last 2 days of closed history + everything
// still open), which is what makes the first load quick.
function adminComplaintDateParams() {
    const from = document.getElementById('cmpFilterFrom')?.value || '';
    const to   = document.getElementById('cmpFilterTo')?.value   || '';
    const params = {};
    if (from) params.fromDate = from;
    if (to)   params.toDate   = to;
    return params;
...
// Table markup lives in complaints.js and is shared with the service admin
// screen — one renderer so the two views can't drift apart.
function renderAdminComplaints() {
    const tbody = document.getElementById('adminComplaintsBody');
    if (!tbody) return;

    const status = document.getElementById('cmpFilterStatus')?.value || '';
    const supId  = document.getElementById('cmpFilterSupervisor')?.value || '';

    const rows = (STATE.adminComplaints || [])
    ...
    tbody.innerHTML = cmpTableRowsHtml(rows, { actionsFor: adminComplaintActionsHtml });
}

// The admin may close either direction — they're the final authority on
// both. (A service admin, by contrast, may only close what they raised.)
...
// Shared by the admin and service admin tables, so it refreshes whichever
// one is actually on screen rather than assuming the admin dashboard.
async function closeTicketPrompt(ticketId) {
    ...
    if (res.ok && res.data?.success) {
        ...
        if (document.getElementById('page-serviceadmin')?.classList.contains('active')) {
...
function exportComplaintsCSV() {
    exportComplaintRowsCSV(STATE.adminComplaints || []);
}

// One exporter for both dashboards. Includes Direction and Assigned To so
// the reverse-flow tickets are distinguishable in the spreadsheet too.
function exportComplaintRowsCSV(rows) {
    if (rows.length === 0) {
        showMessage('No complaints to export', 'error');
        return;
    }
    const headers = [
        'Ticket ID', 'Date Raised', 'Time Raised', 'Raised By', 'Raised By ID',
        'Direction', 'Assigned To', 'Incharge Mobile',
    ...
    const body = rows.map(t => [
        t.ticketId, formatDateDMY(t.dateRaised), t.timeRaised, t.supervisorName, t.supervisorId,
        t.direction === CMP_DIR.TO_SUPERVISOR ? 'For supervisor' : 'For service team',
        t.assignedToName || '', t.inchargeMobile || '',
        t.vlccCode, t.location, t.issue, t.status,
        t.startedBy, formatDateDMY(t.startedDate), t.completedBy, formatDateDMY(t.completedDate),
        t.resolution, t.closedBy, formatDateDMY(t.closedDate), t.daysToClose, t.ageDays
    ]);
    exportToCSV(headers, body, `complaints_${formatDateDisplay(new Date()).replace(/\//g, '-')}.csv`);
