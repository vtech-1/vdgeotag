...
async function loadAdminComplaints() {
    tableLoading('adminComplaintsBody', 9, '⏳ Loading complaints...');

    const res = await apiFetchComplaints();
    if (!res.ok || !Array.isArray(res.data?.rows)) {
        tableLoading('adminComplaintsBody', 9, '⚠️ Failed to load complaints');
        return;
    }
...
function renderAdminComplaints() {
    ...
    if (rows.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="no-records">No complaints found</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = rows.map(t => {
        ...
        // Whoever actually did the work. Falls back through the lifecycle:
        // completed → started → nobody has picked it up yet.
        const solvedBy = t.completedBy || t.startedBy || '--';

        // Ageing flag on anything still open — the number that matters most
        // here is how long a supervisor has been waiting.
        ...
        return `<tr>
            ...
            <td>${loc}<br><small style="color:#888;">VLCC: ${escapeHtml(t.vlccCode || '--')}</small></td>
            ...
            <td style="text-align:center;">${adminComplaintActionsHtml(t)}</td>
        </tr>`;
    }).join('');
}
